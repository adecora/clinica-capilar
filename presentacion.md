<style>
/* General (Estilo Marimo Notebook) */
body {
...
h1 { font-size: 2.25rem !important; font-weight: 800; margin-top: 1.5rem; margin-bottom: 1rem; }
h2 { font-size: 1.5rem !important; margin-top: 2.5rem; margin-bottom: 1rem; border-bottom: none; padding-bottom: 0; }
h3 { font-size: 1.25rem !important; margin-top: 2rem; margin-bottom: 0.75rem; }
h4 { font-size: 1.125rem !important; margin-top: 1.5rem; margin-bottom: 0.75rem; }

/* Celdas de Código (Marimo Vibe) */
pre {
...
p > code, li > code { /* Estilo para código en línea */
  background-color: #f1f5f9;
  color: #db2777; /* Rosa oscuro estilo Markdown moderno */
  padding: 0.15rem 0.3rem;
  border-radius: 0.25rem;
}

/* Imágenes con estilo de ventana */
img:not(.cover-logo) {
  max-width: 100%;
  height: auto;
  border-radius: 0.5rem;
...
code.language-mermaid {
  display: block;
  height: 700px;
  background: transparent; /* Evitar fondo gris del <pre> en el diagrama */
  border: none;
  box-shadow: none;
}
...
/* Notas */
.notas { page-break-after: always; }
.notas h1 { text-align: right; padding-right: 10px; }

/* Indice */
.toc { font-size: 1.125rem; }
.toc a { color: #334155; }
.toc li { margin: 12px 0; }
.toc ul:first-child {
  margin-top: 40px;
  border-left: 2px solid #cbd5e1; /* Línea guía de navegación */
  padding-left: 20px;
}
.toc ul { list-style-type: none; }

/* Referencias */
.references p { font-size: 0.875rem; margin-bottom: 4px; }
...
<div class="cover">
  ...
  <div class="cover-footer">
    ...
    <div>Actualizado: 05-05-2026</div>
  </div>
</div>
...
- El código completo del trabajo se encuentra alojado en el repositorio: [https://github.com/adecora/clinica-capilar](https://github.com/adecora/clinica-capilar).
- La presentación también puede seguirse de forma interactiva como un cuaderno de **marimo**, este cuaderno se ejecuta sobre sqlite para poder desplegarlo en la web sin necesidad de un server: [https://adecora.github.io/clinica-capilar/](https://adecora.github.io/clinica-capilar/).
- Fuentes utilizadas:
  * El material de la asignatura **Modelado de datos**.
  * [Documentación de **mysql**](https://dev.mysql.com/doc/refman/8.4/en/).
...
- [Parte 1: Diseño de base de datos para clínica capilar](#parte-1-diseño-de-base-de-datos-para-clínica-capilar)
  - [Consideraciones iniciales](#consideraciones-iniciales)
  - [Diagrama Entity Relationship](#diagrama-entity-relationship)
  - [Decisiones de diseño y tipos de datos](#decisiones-de-diseño-y-tipos-de-datos)
  - [Queries parte 1](#queries-parte-1)
- [Parte 2: Implementación y consultas sobre modelo relacional dado](#parte-2-implementación-y-consultas-sobre-modelo-relacional-dado)
  - [Querys parte2](#querys-parte2)
...
El siguiente diagrama se puede visualizar de forma interactiva en el repositorios de github, donde se aloja el código de este proyecto y en mi perfile de mermaid:
...
![parte1-query1](assets/parte1-query1.png)

```sql
SELECT * FROM personal_medico;
```

![parte1-query2](assets/parte1-query2.png)

```sql
SELECT * FROM catalogo_medicamentos;
```

![parte1-query3](assets/parte1-query3.png)

```sql
SELECT * FROM facturas;
```

![parte1-query4](assets/parte1-query4.png)
...
![parte1-query5](assets/parte1-query5.png)

```sql
SELECT * FROM trasplantes;
```

![parte1-query6](assets/parte1-query6.png)

```sql
SELECT * FROM equipo_operacion;
```

![parte1-query7](assets/parte1-query7.png)

```sql
SELECT * FROM tratamientos;
```

![parte1-query8](assets/parte1-query8.png)
...
![parte1-query9](assets/parte1-query9.png)

```sql
SELECT * FROM documentacion;
```

![parte1-query10](assets/parte1-query10.png)
...
![parte1-query11](assets/parte1-query11.png)


<div class="page-break"></div>

#### 5. Agrupando al equipo médico en un mismo registro [[1]](#ref1).
...
![parte1-query12](assets/parte1-query12.png)

<div class="page-break"></div>

#### 6. Trazabilidad de las facturas que corresponder a trasplantes [[2]](#ref2).
...
![parte1-query13](assets/parte1-query13.png)
...
![parte1-query14](assets/parte1-query14.png)
...
<img class="modelo-relacional" src="public/modelo-relacional.png" alt="Modelo relacional generado con mysql reverse tool" />
...
![parte2-query1](assets/parte2-query1.png)
...
![parte2-query2](assets/parte2-query2.png)
...
![parte2-query3](assets/parte2-query3.png)
...
![parte2-query4](assets/parte2-query4.png)
...
![parte2-query5](assets/parte2-query5.png)
...
![parte2-query6](assets/parte2-query6.png)
...
![parte2-query7](assets/parte2-query7.png)
...
![parte2-query8](assets/parte2-query8.png)
...
![parte2-query9](assets/parte2-query9.png)
...
![parte2-query10](assets/parte2-query10.png)
...
![parte2-query11](assets/parte2-query11.png)
...
![parte2-query12](assets/parte2-query12.png)
...
![parte2-query13](assets/parte2-query13.png)
...
![parte2-query14](assets/parte2-query14.png)
...
![parte2-query15](assets/parte2-query15.png)
...
![parte2-query16](assets/parte2-query16.png)

```sql
SELECT * FROM salario_maximo_departamento;
```

![parte2-query17](assets/parte2-query17.png)


<div class="page-break"></div>

#### **19.** Crear un procedimiento almacenado que permita obtener los datos solicitados en la pregunta 13 [[3]](#ref3).
...
-- Check del procedure
SELECT ROUTINE_NAME, ROUTINE_TYPE, CREATED FROM information_schema.ROUTINES WHERE ROUTINE_SCHEMA = 'empresa_db';
```

![parte2-query18](assets/parte2-query18.png)

```sql
CALL query_empleados('M', 'ventas');
```

![parte2-query19](assets/parte2-query19.png)
...
![parte2-query20](assets/parte2-query20.png)
...
![parte2-query21](assets/parte2-query21.png)
...
<a id="ref1">[1]</a> Uso de la función de agrupación [`GROUP_CONTAT`](https://dev.mysql.com/doc/refman/8.4/en/aggregate-functions.html#function_group-concat).

<a id="ref2">[2]</a> Explicación del uso de subqueries con [`EXISTS o NOT EXISTS`](https://dev.mysql.com/doc/refman/8.4/en/exists-and-not-exists-subqueries.html).

<a id="ref3">[3]</a> Funcionamiento de los [procedimiento almacenados en **mysql**](https://dev.mysql.com/doc/refman/9.7/en/create-procedure.html).
